<template>
  <div>
    ...
    <p>
      <button @click="select(0)">选中[订单列表]</button>
    </p>
    <p>
      <button @click="select(1)">选中[权限列表]</button>
    </p>
    <p>
      <button @click="select(2)">选中[分配操作权限]</button>
    </p>
  </div>
</template>
      ...
      setActiveMenu1: function (menuList, activeItem) {
        ...
        function loop (list) {
          list.map(function (item) {
            if (JSON.stringify(item).match(activeItem.id)) {
